...
if __name__ == "__main__":
    ...
    try:
        for episode in range(n_episodes):
            ...
            while not done:
                # --- A. 生成随机动作 ---
                # XuanCe 的 DummyVecMultiAgentEnv 要求动作格式为列表:
                # [{agent1: act, agent2: act}, {agent1: act, agent2: act}, ...]
                # 列表长度 = num_envs，每个元素是该环境所有智能体的动作字典
                actions_list = []
                ...
                for env_idx in range(num_envs):
                    ...
                    for agent_id in agents:
                        ...
                        # sample() 返回单个动作，适用于当前环境
                        actions_dict[agent_id] = act_space.sample()
                    
                    actions_list.append(actions_dict)

                # --- B. 环境步进 ---
                next_obs, rewards, terminated, truncated, info = envs.step(actions_list)
